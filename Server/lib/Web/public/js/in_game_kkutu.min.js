(function() {
    ...
    */
    ...
    function applyOptions(a) {
        $data.opts = a, $data.muteBGM = $data.opts.mb, $data.muteEff = $data.opts.me, $("#mute-bgm").attr("checked", $data.muteBGM), $("#mute-effect").attr("checked", $data.muteEff), $("#deny-invite").attr("checked", $data.opts.di), $("#deny-whisper").attr("checked", $data.opts.dw), $("#deny-friend").attr("checked", $data.opts.df), $("#auto-ready").attr("checked", $data.opts.ar), $("#sort-user").attr("checked", $data.opts.su), $("#only-waiting").attr("checked", $data.opts.ow), $("#only-unlock").attr("checked", $data.opts.ou), $data.bgm && ($data.muteBGM ? ($data.bgm.volume = 0, $data.bgm.stop()) : ($data.bgm.volume = 1, $data.bgm = playBGM($data.bgm.key, !0)))
    }
    ...
    function onMessage(a) {
        ...
        switch (a.type) {
            case "welcome":
                $data.id = a.id, $data.guest = a.guest, $data.admin = a.admin, $data.users = a.users, $data.robots = {}, $data.rooms = a.rooms, $data.place = 0, $data.friends = a.friends, $data._friends = {}, $data._playTime = a.playTime, $data._okg = a.okg, $data._gaming = !1, $data.box = a.box, a.test && alert(L.welcomeTestServer), location.hash[1] && tryJoin(location.hash.slice(1)), updateUI(void 0, !0), welcome(), a.caj && checkAge(), updateCommunity(), Object.defineProperty($data, "_testt", {
                    value: addInterval(function() {
                        $stage.talk.val() != $data._ttv && (cheatDetection({
                            type: "chat",
                            value: $stage.talk.val()
                        }, !0), tailuserEnabled && send("cheatreport", {
                            ev: "c",
                            v: $stage.talk.val()
            ...
            case "disconn":
                ...
                break;
            case "connRoom":
                $data._preQuick && (playSound("success"), $stage.dialog.quick.hide(), delete $data._preQuick), $stage.dialog.quick.hide(), $data.setUser(a.user.id, a.user), c = $data.usersR[a.user.id] = a.user, c.id == $data.id ? loading() : notice((c.profile.title || c.profile.name) + L.hasJoined), updateUserList();
                break;
            case "disconnRoom":
            ...
            case "friends":
                $data._friends = {};
                for (b in a.list) a.list[b].forEach(function(a) {
                    $data._friends[a] = {
                        server: b
                    }
                });
            ...
            case "friend":
                ...
                break;
            case "friendAdd":
                c = $data.users[a.from].profile, b = (c.title || c.name) + "(#" + a.from.substr(0, 5) + ")", send("friendAddRes", {
                    from: a.from,
                    res: !$data.opts.df && confirm(b + L.attemptFriendAdd)
                }, !0);
                break;
            case "friendAddRes":
                c = $data.users[a.target].profile, b = (c.title || c.name) + "(#" + a.target.substr(0, 5) + ")", notice(b + L["friendAddRes_" + (a.res ? "ok" : "no")]), a.res && ($data.friends[a.target] = c.title || c.name, $data._friends[a.target] = {
                    server: $data.server
                }, updateCommunity());
                break;
            case "friendEdit":
            ...
            case "turnEnd":
                ...
                break;
            case "roundEnd":
                for (b in a.users) $data.setUser(b, a.users[b]);
            ...
            case "invited":
                ...
                break;
            case "inviteNo":
                c = $data.users[a.target], notice((c.profile.title || c.profile.name) + L.inviteDenied);
                break;
            case "okg":
            ...
            case "obtain":
                ...
                break;
            case "expired":
                for (b in a.list) notice(iName(a.list[b]) + L.hasExpired);
                break;
            case "blocked":
                notice(L.blocked);
                break;
            case "test":
                tailuserEnabled = !tailuserEnabled;
                break;
            case "setNick":
                showDialog($stage.dialog.setNick);
                break;
            case "error":
                if (b = a.message || "", 401 == a.code);
                ...
                } else if (409 == a.code) b = L["server_" + b];
                else {
                    ...
                    if (413 == a.code) $stage.dialog.room.hide(), $stage.menu.setRoom.trigger("click");
                        ...
                        if (b = a.message, -1 != b.indexOf("생년월일")) {
    ...
    function welcome() {
        playBGM("lobby"), $("#Intro").animate({
            ...
            $("#Intro").hide()
        }, 2e3), $data.admin && console.log("관리자 모드")
    }
    ...
    function toggleWhisperBlock(a) {
        $data._wblock.hasOwnProperty(a) ? (delete $data._wblock[a], notice(a + L.wnblocked)) : ($data._wblock[a] = !0, notice(a + L.wblocked))
    }

    function toggleShutBlock(a) {
        $data._shut.hasOwnProperty(a) ? (delete $data._shut[a], notice(a + L.userNShut)) : ($data._shut[a] = !0, notice(a + L.userShut))
    }
    ...
    function processRoom(a) {
        ...
        if (a.myRoom = $data.place == a.room.id || a.target == $data.id, a.myRoom) {
            if ($target = $data.users[a.target], a.kickVote && (notice(getKickText($target.profile, a.kickVote)), $target.id == a.id && alert(L.hasKicked)), -1 == a.room.players.indexOf($data.id)) $data.room && $data.room.gaming && (stopAllSounds(), $data.practicing = !1, $data._gaming = !1, $stage.box.room.height(360), playBGM("lobby")), $data.users[$data.id].game.ready = !1, $data.users[$data.id].game.team = 0, $data.users[$data.id].game.form = "J", $stage.menu.spectate.removeClass("toggled"), $stage.menu.ready.removeClass("toggled"), $data.room = null, $data.resulting = !1, $data._players = null, $data._master = null, $data.place = 0, a.room.practice && (delete $data.users[0], $data.room = $data._room, $data.place = $data._place, $data.master = $data.__master, $data._players = $data.__players, delete $data._room);
    ...
    function addonNickname(a, b) {
        b.equip.NIK && a.addClass("x-" + b.equip.NIK), "b1_gm" == b.equip.BDG && a.addClass("x-gm"), "b1_planner" == b.equip.BDG && a.addClass("x-planner"), "b1_designer" == b.equip.BDG && a.addClass("x-designer"), "b1_bgm" == b.equip.BDG && a.addClass("x-bgm"), "b1_video" == b.equip.BDG && a.addClass("x-video"), "b1_bj" == b.equip.BDG && a.addClass("x-bj"), "b1_yt" == b.equip.BDG && a.addClass("x-yt"), "b1_tw" == b.equip.BDG && a.addClass("x-tw"), "b1_word" == b.equip.BDG && a.addClass("x-word"), "vip" == b.equip.BDG && a.addClass("x-vip")
    }
    ...
    function roomListBar(a) {
        ...
        }), a.gaming && b.addClass("rooms-gaming"), a.password && b.addClass("rooms-locked"), b
    }
    ...
    function updateRoom(a) {
        ...
        if (setRoomHead($(".RoomBox .product-title"), $data.room), setRoomHead($(".GameBox .product-title"), $data.room), a) {
            ...
            clearTimeout($data._jamsu), delete $data._jamsu
        } else {
    ...
    function drawMyDress(a) {
        ...
        renderMoremi(b, c.equip), $(".dress-type.selected").removeClass("selected"), $("#dress-type-all").addClass("selected"), $("#dress-exordial").val(c.exordial), drawMyGoods(a || !0)
    }
    ...
    function requestProfile(a) {
        ...
        if ($("#ProfileDiag .dialog-title").html((e.profile.title || e.profile.name) + L.sProfile), $(".profile-head").empty().append(b = $("<div>").addClass("moremi profile-moremi")).append($("<div>").addClass("profile-head-item").append(getImage(e.profile.image).addClass("profile-image")).append($("<div>").addClass("profile-title ellipse").html(e.profile.title || e.profile.name).append($("<label>").addClass("profile-tag").html(" #" + e.id.toString().substr(0, 5))))).append($("<div>").addClass("profile-head-item").append(getLevelImage(e.data.score).addClass("profile-level")).append($("<div>").addClass("profile-level-text").html(L.LEVEL + " " + (d = getLevel(e.data.score)))).append($("<div>").addClass("profile-score-text").html(commify(e.data.score) + " / " + commify(EXP[d - 1]) + L.PTS))).append(c = $("<div>").addClass("profile-head-item profile-exordial ellipse").text(badWords(e.exordial || "")).append($("<div>").addClass("expl").css({
                ...
                width: 300,
                "font-size": "11px"
            }).text(e.exordial))), e.robot) $stage.dialog.profileLevel.show(), $stage.dialog.profileLevel.prop("disabled", $data.id != $data.room.master), $("#profile-place").html($data.room.id + L.roomNumber);
        else {
            $stage.dialog.profileLevel.hide(), $("#profile-place").html(e.place ? e.place + L.roomNumber : L.lobby);
    ...
    function gameReady() {
        var a, b;
        for (a in $data.room.players) b = $data._replay ? $rec.users[$data.room.players[a]] || $data.room.players[a] : $data.users[$data.room.players[a]] || $data.robots[$data.room.players[a].id], b.game.score = 0, delete $data["_s" + $data.room.players[a]];
        delete $data.lastFail, $data.failCombo = 0, $data._spectate = -1 == $data.room.game.seq.indexOf($data.id), $data._gAnim = !0, $stage.box.room.show().height(360).animate({
            height: 1
    ...
    function replayStop() {
        delete $data.room, $data._replay = !1, $stage.box.room.height(360), clearTimeout($data._rt), updateUI(), playBGM("lobby")
    }
    ...
    function getLevelImage(a) {
        var b = getLevel(a) - 1,
            c = b % 25 * -100,
            d = -100 * Math.floor(.04 * b);
        return $("<div>").css({
            float: "left",
    ...
    function stopBGM() {
        $data.bgm && ($data.bgm.stop(), delete $data.bgm)
    }
    ...
    function chatBalloon(a, b, c) {
        ...
        var d, e, f = (2 & c ? $("#game-user-" + b) : $("#room-user-" + b)).offset(),
            ...
            h = $("<div>").addClass("chat-balloon").attr("id", "cb-" + b).append($("<div>").addClass("jt-image " + g))[2 == c ? "prepend" : "append"]($("<h4>").text(a));
    ...
    function chat(a, b, c, d) {
        ...
        if (!$data._shut[a.title || a.name]) {
            ...
            b = badWords(b), playSound("k"), stackChat(), !mobile && $data.room && (e = ($data.room.gaming ? 2 : 0) + ($(".jjoriping").hasClass("cw") ? 1 : 0), chatBalloon(b, a.id, e)), $stage.chat.append(g = $("<div>").addClass("chat-item").append(e = $("<div>").addClass("chat-head ellipse").text(a.title || a.name)).append(f = $("<div>").addClass("chat-body").text(b)).append($("<div>").addClass("chat-stamp").text(i.toLocaleTimeString()))), d && e.prepend($("<i>").addClass("fa fa-video-camera")), e.on("click", function(b) {
                requestProfile(a.id)
    ...
    function notice(a, b) {
        var c = new Date;
        playSound("k"), stackChat(), $("#Chat,#chat-log-board").append($("<div>").addClass("chat-item chat-notice").append($("<div>").addClass("chat-head").text(b || L.notice)).append($("<div>").addClass("chat-body").html(a)).append($("<div>").addClass("chat-stamp").text(c.toLocaleTimeString()))), $stage.chat.scrollTop(999999999), "tail" == b && console.warn(c.toLocaleString(), a)
    }
    ...
    function renderMoremi(a, b) {
        ...
        (d = b.BDG) && e.append($("<img>").addClass("moremies moremi-badge").attr("src", iImage(d)).css({
            width: "100%",
            height: "100%"
        })), e.children(".moremi-back").after($("<img>").addClass("moremies moremi-body").attr("src", b.robot ? "/img/kkutu/moremi/robot.png" : "/img/kkutu/moremi/body.png").css({
            width: "100%",
            height: "100%"
    ...
    var MODE, BEAT = [null, "10000000", "10001000", "10010010", "10011010", "11011010", "11011110", "11011111", "11111111"],
        ...
        MOREMI_PART, AVAIL_EQUIP, RULE, OPTIONS, MAX_LEVEL = 360,
        TICK = 30,
        EXP = [],
        BAD = new RegExp(["느으*[^가-힣]*금마?", "니[^가-힣]*(엄|앰|엠)", "(ㅄ|ㅅㅂ|ㅂㅅ)", "미친(년|놈)?", "(병|븅|빙)[^가-힣]*신", "보[^가-힣]*지", "(새|섀|쌔|썌)[^가-힣]*(기|끼)", "섹[^가-힣]*스", "(시|씨|쉬|쒸)이*입?[^가-힣]*(발|빨|벌|뻘|팔|펄)", "십[^가-힣]*새", "씹", "(애|에)[^가-힣]*미", "자[^가-힣]*지", "존[^가-힣]*나", "좆|죶", "지랄", "창[^가-힣]*(녀|년|놈)", "fuck", "sex"].join("|"), "g"),
        ws, rws, $stage, $sound = {},
        $_sound = {},
        $data = {},
        $lib = {
    ...
    $(document).ready(function() {
        ...
        function h() {
            ...
            ws = new _WebSocket($data.URL), ws.onopen = function(a) {
                loading()
            }, ws.onmessage = _onMessage = function(b) {
                var c = JSON.parse(b.data);
                if ("recaptcha" === c.type) {
                    var d = $("#intro-text");
                    return d.empty(), d.html("게스트는 캡챠 인증이 필요합니다.<br/>로그인을 하시면 캡챠 인증을 건너뛰실 수 있습니다.<br/><br/>"), d.append($('<div class="g-recaptcha" id="recaptcha" style="display: table; margin: 0 auto;"></div>')), void grecaptcha.render("recaptcha", {
                        sitekey: c.siteKey,
                        callback: a
                    })
                }
        ...
        loadSounds($data._soundList, function() {
            processShop(h)
        }), delete $data._soundList, MOREMI_PART = $("#MOREMI_PART").html().split(","), AVAIL_EQUIP = $("#AVAIL_EQUIP").html().split(","), RULE = JSON.parse($("#RULE").html()), OPTIONS = JSON.parse($("#OPTIONS").html()), MODE = Object.keys(RULE), mobile = "true" == $("#mobile").html(), mobile && (TICK = 200), $data._timePercent = function() {
            ...
            null == b ? (delete $data.rooms[a], c && $("#room-" + a).remove()) : (c && !$data.rooms[a] && $stage.lobby.roomList.append($("<div>").attr("id", "room-" + a)), $data.rooms[a] = b, c && $("#room-" + a).replaceWith(roomListBar(b)))
        }, $data.setUser = function(a, b) {
            var c, d = getOnly(),
                e = "for-lobby" == d || "for-master" == d;
            if ($data._replay) return void($rec.users[a] = b);
            null == b ? (delete $data.users[a], e && $("#users-item-" + a + ",#invite-item-" + a).remove()) : (e && !$data.users[a] && (c = userListBar(b, "for-master" == d), "for-master" == d ? $stage.dialog.inviteList.append(c) : $stage.lobby.userList.append(c)), $data.users[a] = b, e && (c ? $("#" + c.attr("id")).replaceWith(c) : $("#" + ("for-lobby" == d ? "users-item-" : "invite-item") + a).replaceWith(userListBar(b, "for-master" == d))))
            ...
            b && ("/" == c.value[0] ? (c.cmd = c.value.split(" "), runCommand(c.cmd)) : (($stage.game.here.is(":visible") || $data._relay) && (c.relay = !0), send("talk", c)), $data._whisper ? ($stage.talk.val("/e " + $data._whisper + " "), delete $data._whisper) : $stage.talk.val(""), $stage.game.hereText.val(""))
        ...
        for ($stage.menu.help.on("click", function(a) {
                ...
                $data.typeRoom = "setRoom", $("#room-title").val($data.room.title), $("#room-limit").val($data.room.limit), $("#room-mode").val($data.room.mode).trigger("change"), $("#room-round").val($data.room.round), $("#room-time").val($data.room.time / e.time);
                ...
                send("leave")
            }), $stage.dialog.setNickOK.on("click", function(e) {
                let nick = $("#nick").val().split(" ").join("_").replace(new RegExp("(GM|관리자|관리)", "ig"), "")
                if (nick.length > 10) return alert('닉네임은 10자 이하이어야 합니다.')
                else {
                  $stage.dialog.setNick.hide()
                  ws.send(JSON.stringify({
                      type: "setNick",
                      value: nick
                  }))
            ...
            }), $stage.menu.bulletin.on("click", function(a) {
                $("#bulletin-board").attr("src", "/kkt_notice.html"), showDialog($stage.dialog.bulletin)
                ...
                }), $.cookie("kks", JSON.stringify($data.opts)), $stage.dialog.setting.hide()
                ...
                $data.practicing && ($data.room.gaming = !0, send("leave")), $data.resulting = !1, $stage.dialog.result.hide(), delete $data._replay, delete $data._resultRank, $stage.box.room.height(360), playBGM("lobby"), forkChat(), updateUI()
                ...
                var b = new Date($rec.time),
                    ...
                    e = "KKuTu" + b.getFullYear() + "-" + (b.getMonth() + 1) + "-" + b.getDate() + " " + b.getHours() + "-" + b.getMinutes() + "-" + b.getSeconds() + ".kkt",
                    f = $("<a>").attr({
                        download: e,
                        href: d
                    }).on("click", function(a) {
        ...
        delete $data._spaced, $("body").off("keydown", "#" + $data._chatter.attr("id"), onSpace)
    ...
    var spamWarning = 0,
        spamCount = 0,
        BETWEEN_CHAT_MINIMUM_MILLS = 25,
        MINIMUM_CHAR_COUNT_PER_CHAT = 4,
        KEY_CODES = {
            F12: 123,
            Backspace: 8,
            VK_PACKET: 231
    ...
    String.prototype.toKorChars = function() {
        for (var a, b, c, d, e = this, f = e.length, g = [], h = 0; h < f; h++) 32 !== (d = e.charCodeAt(h)) && (d < 44032 || d > 55203 ? g.push(e.charAt(h)) : (d = e.charCodeAt(h) - 44032, c = d % 28, b = (d - c) / 28 % 21, a = ((d - c) / 28 - b) / 21, g.push(cCho[a], cJung[b]), "" !== cJong[c] && g.push(cJong[c])));
        return g
    }, invoke(function() {
        var a = {},
            b = function(b) {
                ...
                switch (b.type) {
                    case "keydown":
                        var e = b.value;
                        if (!a.lastKey || !a.keyTime) {
                            a.keyTime = d, a.lastKey = e;
                            break
                        }
                        if (e === KEY_CODES.F12 && c("F12(개발자 도구)를 사용하였습니다.", !1), d - a.keyTime <= BETWEEN_CHAT_MINIMUM_MILLS) {
                            if (a.lastKey === KEY_CODES.Backspace) break;
                            if (a.lastKey === e) break;
                            c(BETWEEN_CHAT_MINIMUM_MILLS + "ms 내에 연속적으로 입력하였습니다.", !1)
                        }
                        e === KEY_CODES.VK_PACKET && c("가상 키보드(VK_PACKET)가 감지되었습니다.", !1), a.lastKey = e, a.keyTime = d, a.lastAction = "keydown";
                        break;
                    case "chat":
                        var f = b.value;
                        if (!a.lastChat) {
                        ...
                        f.includes(".macro") && c("배포되어 있는 특정 매크로 작동을 시도하였습니다. (" + f + ")", !0), f.length - a.lastChat.length >= MINIMUM_CHAR_COUNT_PER_CHAT && c("한번에 " + MINIMUM_CHAR_COUNT_PER_CHAT + "글자 이상을 입력하였습니다.", !0), a.lastChat = f, a.lastAction = "chat";
                        break;
                    case "keyup":
                        var g = b.value;
                        a.lastKey !== g && c("keyown과 keyup가 같지 않습니다.", !1)
                }
            };
