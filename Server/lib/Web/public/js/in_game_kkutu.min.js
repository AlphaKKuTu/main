...
function updateMe(){var a,b=$data.users[$data.id],c=0,d=getLevel(b.data.score),e=EXP[d-2]||0,f=EXP[d-1];for(a in b.data.record)c+=b.data.record[a][1];renderMoremi(".my-image",b.equip),$(".my-stat-level").replaceWith(getLevelImage(b.data.score).addClass("my-stat-level")),$(".my-stat-name").html(b.profile.title||b.profile.name),$(".my-stat-record").html(L.globalWin+" "+c+L.W),$(".my-stat-ping").html(commify(b.money)+L.ping),$(".my-okg .graph-bar").width($data._playTime%6e5/6e3+"%"),$(".my-okg-text").html(prettyTime($data._playTime)),$(".my-level").html(L.LEVEL+" "+d),$(".my-gauge .graph-bar").width((b.data.score-e)/(f-e)*190),$(".my-gauge-text").html(commify(b.data.score)+" / "+commify(f))}
function prettyTime(a){var b=Math.floor(a/6e4)%60,c=Math.floor(.001*a)%60,d=Math.floor(a/36e5),e=[];return d&&e.push(d+L.HOURS),b&&e.push(b+L.MINUTE),d||e.push(c+L.SECOND),e.join(" ")}
function updateUserList(a){var b,c,d,e=0;if($data.opts.su){d=[];for(b in $data.users)e++,d.push($data.users[b]);d.sort(function(a,b){return b.data.score-a.data.score}),a=!0}else{d=$data.users;for(b in $data.users)e++}
if($stage.lobby.userListTitle.html("<i class='fa fa-users'></i>&lt;<b>"+L["server_"+$data.server]+"</b>&gt; "+L.UserList.replace("FA{users}","")+" ["+e+L.MN+"]"),a){$stage.lobby.userList.empty(),$stage.dialog.inviteList.empty();for(b in d)c=d[b],c.robot||($stage.lobby.userList.append(userListBar(c)),0==c.place&&$stage.dialog.inviteList.append(userListBar(c,!0)))}}
function userListBar(a,b){var c;return c=b?$("<div>").attr("id","invite-item-"+a.id).addClass("invite-item users-item").append($("<div>").addClass("jt-image users-image").css("background-image","url('"+a.profile.image+"')")).append(getLevelImage(a.data.score).addClass("users-level")).append($("<div>").addClass("users-name").html(a.profile.title||a.profile.name)).on("click",function(a){requestInvite($(a.currentTarget).attr("id").slice(12))}):$("<div>").attr("id","users-item-"+a.id).addClass("users-item").append($("<div>").addClass("jt-image users-image").css("background-image","url('"+a.profile.image+"')")).append(getLevelImage(a.data.score).addClass("users-level")).append($("<div>").addClass("users-name ellipse").html(a.profile.title||a.profile.name)).on("click",function(a){requestProfile($(a.currentTarget).attr("id").slice(11))}),addonNickname(c,a),c}
function addonNickname(a,b){b.equip.NIK&&a.addClass("x-"+b.equip.NIK),"b1_gm"==b.equip.BDG&&a.addClass("x-gm"),"b1_planner"==b.equip.BDG&&a.addClass("x-planner"),"b1_designer"==b.equip.BDG&&a.addClass("x-designer"),"b1_bgm"==b.equip.BDG&&a.addClass("x-bgm"),"b1_video"==b.equip.BDG&&a.addClass("x-video"),"b1_bj"==b.equip.BDG&&a.addClass("x-bj"),"b1_yt"==b.equip.BDG&&a.addClass("x-yt"),"b1_tw"==b.equip.BDG&&a.addClass("x-tw"),"b1_word"==b.equip.BDG&&a.addClass("x-word"),"vip"==b.equip.BDG&&a.addClass("x-vip")}
function updateRoomList(a){function b(a){$stage.menu.newRoom.trigger("click")}
var c,d=0;if(a){$stage.lobby.roomList.empty();for(c in $data.rooms)$stage.lobby.roomList.append(roomListBar($data.rooms[c])),d++}else{$(".rooms-create").remove();for(c in $data.rooms)d++}
...
function requestInvite(a){var b;("AI"==a||(b=$data.users[a].profile.title||$data.users[a].profile.name,confirm(b+L.sureInvite)))&&send("invite",{target:a})}
function checkFailCombo(a){$data._replay||$data.lastFail!=$data.id||$data.id!=a?$data.failCombo=0:($data.failCombo++,1==$data.failCombo&&notice(L.trollWarning),$data.failCombo>1&&(send("leave"),fail(437))),$data.lastFail=a}
function clearGame(){$data._spaced&&$lib.Typing.spaceOff(),clearInterval($data._tTime),$data._relay=!1}
function gameReady(){var a,b;for(a in $data.room.players)b=$data._replay?$rec.users[$data.room.players[a]]||$data.room.players[a]:$data.users[$data.room.players[a]]||$data.robots[$data.room.players[a].id],b.game.score=0,delete $data["_s"+$data.room.players[a]];delete $data.lastFail,$data.failCombo=0,$data._spectate=-1==$data.room.game.seq.indexOf($data.id),$data._gAnim=!0,$stage.box.room.show().height(360).animate({height:1},500),$stage.box.game.height(1).animate({height:410},500),stopBGM(),$stage.dialog.resultSave.attr("disabled",!1),clearBoard(),$stage.game.display.html(L.soon),playSound("game_start"),forkChat(),addTimeout(function(){$stage.box.room.height(360).hide(),$stage.chat.scrollTop(999999999)},500)}
function replayPrevInit(){var a;for(a in $data.room.game.seq)$data.room.game.seq[a].robot&&($data.room.game.seq[a].game.score=0);$rec.users={};for(a in $rec.players){var b=$rec.players[a].id,c=$rec.readies[b]||{},d=$data.users[b]||$data.robots[b],e=b;$rec.players[a].robot?(d=$rec.users[b]={robot:!0},e=$rec.players[a],e.game={}):d=$rec.users[b]={},$data.room.players.push(e),d.id=e,d.profile=$rec.players[a],d.data=d.profile.data,d.equip=d.profile.equip,d.game={score:0,team:c.t}}
$data._rf=0}
function replayReady(){var a;replayStop(),$data._replay=!0,$data.room={title:$rec.title,players:[],events:[],time:$rec.roundTime,round:$rec.round,mode:$rec.mode,limit:$rec.limit,game:$rec.game,opts:$rec.opts,readies:$rec.readies},replayPrevInit();for(a in $rec.events)$data.room.events.push($rec.events[a]);$stage.box.userList.hide(),$stage.box.roomList.hide(),$stage.box.game.show(),$stage.dialog.replay.hide(),gameReady(),updateRoom(!0),$data.$gp=$(".GameBox .product-title").empty().append($data.$gpt=$("<div>").addClass("game-replay-title")).append($data.$gpc=$("<div>").addClass("game-replay-controller").append($("<button>").html(L.replayNext).on("click",replayNext)).append($("<button>").html(L.replayPause).on("click",replayPause)).append($("<button>").html(L.replayPrev).on("click",replayPrev))),$data._gpp=L.replay+" - "+new Date($rec.time).toLocaleString(),$data._gtt=$data.room.events[$data.room.events.length-1].time,$data._eventTime=0,$data._rt=addTimeout(replayTick,2e3),$data._rprev=0,$data._rpause=!1,replayStatus()}
function replayPrev(a){var b,c,d=$data.room.events[--$data._rf];if(d){b=d.time;do{if(!(d=$data.room.events[--$data._rf]))break}while(b-d.time<1e3);for(c=$data._rf-1,replayPrevInit(),b=$data.muteEff,$data.muteEff=!0,i=0;i<c;i++)replayTick();$(".deltaScore").remove(),$data.muteEff=b,replayTick()}}
function replayPause(a){var b=$data._rpause=!$data._rpause;$(a.target).html(b?L.replayResume:L.replayPause)}
function replayNext(a){clearTimeout($data._rt),replayTick()}
function replayStatus(){$data.$gpt.html($data._gpp+" ("+(.001*$data._eventTime).toFixed(1)+L.SECOND+" / "+(.001*$data._gtt).toFixed(1)+L.SECOND+")")}
function replayTick(a){var b,c=$data.room.events[$data._rf];return clearTimeout($data._rt),a||$data._rf++,c?$data._rpause?($data._rf--,$data._rt=addTimeout(replayTick,100)):(b=c.data,b.hint&&(b.hint={_id:b.hint}),"chat"==b.type&&(b.timestamp=$rec.time+c.time),onMessage(b),$data._eventTime=c.time,replayStatus(),void($data.room.events.length>$data._rf?$data._rt=addTimeout(replayTick,$data.room.events[$data._rf].time-c.time):replayStop())):void replayStop()}
function replayStop(){delete $data.room,$data._replay=!1,$stage.box.room.height(360),clearTimeout($data._rt),updateUI(),playBGM("lobby")}
function startRecord(a){var b,c;$rec={version:$data.version,me:$data.id,players:[],events:[],title:$data.room.title,roundTime:$data.room.time,round:$data.room.round,mode:$data.room.mode,limit:$data.room.limit,game:$data.room.game,opts:$data.room.opts,readies:$data.room.readies,time:(new Date).getTime()};for(b in $data.room.players){var d;c=$data.users[$data.room.players[b]]||$data.room.players[b],d={id:c.id,score:0},c.robot?(d.id=c.id,d.robot=!0,d.data={score:0},c={profile:getAIProfile(c.level)}):(d.data=c.data,d.equip=c.equip),d.title="#"+c.id,$rec.players.push(d)}
$data._record=!0}
function stopRecord(){$data._record=!1}
function recordEvent(a){if(!$data._replay&&$rec){var b,c=a;if(a.hasOwnProperty("type")&&"room"!=a.type&&"obtain"!=a.type){a={};for(b in c)a[b]=c[b];a.profile&&(a.profile={id:a.profile.id,title:"#"+a.profile.id}),a.user&&(a.user={id:a.user.profile.id,profile:{id:a.user.profile.id,title:"#"+a.user.profile.id},data:{score:0},equip:{}}),$rec.events.push({data:a,time:(new Date).getTime()-$rec.time})}}}
function clearBoard(){$data._relay=!1,loading(),$stage.game.here.hide(),$stage.dialog.result.hide(),$stage.dialog.dress.hide(),$stage.dialog.charFactory.hide(),$(".jjoriping,.rounds,.game-body").removeClass("cw"),$(".jjoriping,.rounds").removeClass("dg"),$(".rounds").removeClass("painter"),$stage.game.display.empty(),$stage.game.chain.hide(),$stage.game.hints.empty().hide(),$stage.game.tools.hide(),$stage.game.cwcmd.hide(),$stage.game.bb.hide(),$stage.game.round.empty(),$stage.game.history.empty(),$stage.game.items.show().css("opacity",0),$(".jjo-turn-time .graph-bar").width(0).css({float:"","text-align":"","background-color":""}),$(".jjo-round-time .graph-bar").width(0).css({float:"","text-align":""}).removeClass("round-extreme"),$(".game-user-bomb").removeClass("game-user-bomb")}
function drawRound(a){var b;for($stage.game.round.empty(),b=0;b<$data.room.round;b++)$stage.game.round.append($l=$("<label>").html($data.room.game.title[b])),b+1==a&&$l.addClass("rounds-current")}
function turnGoing(){route("turnGoing")}
function turnHint(a){route("turnHint",a)}
function turnError(a,b){$stage.game.display.empty().append($("<label>").addClass("game-fail-text").text((L["turnError_"+a]?L["turnError_"+a]+": ":"")+b)),playSound("fail"),clearTimeout($data._fail),$data._fail=addTimeout(function(){$stage.game.display.html($data._char)},1800)}
function getScore(a){return $data._replay?$rec.users[a].game.score:($data.users[a]||$data.robots[a]).game.score}
function addScore(a,b){$data._replay?$rec.users[a].game.score+=b:($data.users[a]||$data.robots[a]).game.score+=b}
function drawObtainedScore(a,b){return a.append(b),addTimeout(function(){b.remove()},2e3),a}
function turnEnd(a,b){route("turnEnd",a,b)}
